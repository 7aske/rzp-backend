...
public class PostCountFilter implements Filter {
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
		...
		Specification<Post> specification = (Specification<Post>) converter.convert(query);
		response.setHeader(HEADER_NAME, postService.count(specification).toString());
		filterChain.doFilter(servletRequest, servletResponse);
	}
	...
	private void initPostRepository(ServletRequest request) {
		if (postService == null) {
			...
			postService = webApplicationContext.getBean(PostService.class);
		}
	}
}
